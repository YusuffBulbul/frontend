    ...
    return (
        <Stack spacing={3}>
                ...
                <CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}>
                    <Box sx={{ alignItems: { md: 'center' }, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3, justifyContent: 'space-between' }}>
                        ...
                        <Stack spacing={1.25} sx={{ alignItems: { xs: 'stretch', sm: 'flex-start' } }}>
                            ...
                            <Button component={Link} startIcon={<AddTaskOutlinedIcon />} to="/tasks" variant="outlined">
                                Create a task
                            </Button>
                        </Stack>
                    </Box>
            ...
            {!error && (
                <>
                    <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', lg: 'repeat(4, 1fr)' } }}>
                        <DashboardSummaryCard accent="#e8f0fe" icon={<AssignmentOutlinedIcon color="primary" />} label="Total tasks" value={metrics.total} />
                        <DashboardSummaryCard accent="#eef4ff" icon={<PendingActionsOutlinedIcon color="primary" />} label="To do" value={metrics.todo} />
                        <DashboardSummaryCard accent="#fff4e5" icon={<PendingActionsOutlinedIcon color="warning" />} label="In progress" value={metrics.inProgress} />
                        <DashboardSummaryCard accent="#edf7ed" icon={<CheckCircleOutlinedIcon color="success" />} label="Completed" value={metrics.completed} />
                    </Box>
